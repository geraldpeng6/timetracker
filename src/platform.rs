...
fn get_active_window_fallback() -> anyhow::Result<WindowInfo> {
    #[cfg(target_os = "windows")]
    {
        ...
        monitor.get_active_window()?.ok_or_else(|| anyhow::anyhow!("No active window found"))
    }
...
mod windows {
    ...
    impl super::WindowMonitor for WindowsMonitor {
        fn get_active_window(&mut self) -> anyhow::Result<Option<super::WindowInfo>> {
            ...
            let app_name = if let Some(process) = self.system.process(sysinfo::Pid::from_u32(process_id)) {
...
mod macos {
    ...
    impl super::WindowMonitor for MacOSMonitor {
        fn get_active_window(&mut self) -> anyhow::Result<Option<super::WindowInfo>> {
            ...
            // 简化实现：只返回一个基本的窗口信息，避免使用可能导致权限问题的 AppleScript
            // 这是一个安全的回退实现
            Ok(Some(super::WindowInfo {
                app_name: "Active Application".to_string(),
