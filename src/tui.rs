...
impl TuiApp {
    ...
    fn ui<B: Backend>(&mut self, f: &mut Frame) {
        ...
        // 渲染标题和标签页
        self.render_header::<B>(f, chunks[0]);

        // 渲染主内容
        if self.show_help {
            self.render_help::<B>(f, chunks[1]);
        } else {
            match self.current_tab {
                TabIndex::Overview => self.render_overview::<B>(f, chunks[1]),
                TabIndex::Applications => self.render_applications::<B>(f, chunks[1]),
                TabIndex::Windows => self.render_windows::<B>(f, chunks[1]),
                TabIndex::Recent => self.render_recent::<B>(f, chunks[1]),
            }
        }

        // 渲染状态栏
        self.render_status_bar::<B>(f, chunks[2]);
    }

    fn render_header<B: Backend>(&self, f: &mut Frame, area: Rect) {
        ...
        let tabs = Tabs::new(titles)
            .block(Block::default().borders(Borders::ALL).title("TimeTracker 统计"))
    ...
    fn render_overview<B: Backend>(&mut self, f: &mut Frame, area: Rect) {
        let chunks = Layout::default()
            .direction(Direction::Vertical)
            .constraints([
        ...
        // 总体统计
        self.render_total_stats::<B>(f, chunks[0]);

        // 当前活动
        self.render_current_activity::<B>(f, chunks[1]);

        // 最近使用的应用（前5个）
        self.render_top_apps::<B>(f, chunks[2], 5);
    }

    fn render_total_stats<B: Backend>(&self, f: &mut Frame, area: Rect) {
    ...
    fn render_current_activity<B: Backend>(&self, f: &mut Frame, area: Rect) {
        let text = if let Some(current) = &self.tracker.current_activity {
            ...
            vec![
                ...
                Line::from(vec![
                    ...
                    Span::styled(
                        current.start_time.with_timezone(&Local).format("%H:%M:%S").to_string(),
                        Style::default().fg(Color::White),
                    ),
                ]),
            ]
    ...
    fn render_top_apps<B: Backend>(&mut self, f: &mut Frame, area: Rect, limit: usize) {
        ...
        let items: Vec<ListItem> = sorted_apps
            ...
            .map(|(i, (app_name, duration))| {
                ...
                ListItem::new(Line::from(vec![
                    ...
                    Span::styled((*app_name).clone(), Style::default().fg(color).add_modifier(Modifier::BOLD)),
                    Span::styled(
                        format!(" - {}h {}m {}s", hours, minutes, seconds),
                        Style::default().fg(Color::Gray),
                    ),
        ...
        let list = List::new(items)
            .block(Block::default().borders(Borders::ALL).title(format!("最常用应用 (前{})", limit)))
            .highlight_style(Style::default().bg(Color::DarkGray));

        f.render_stateful_widget(list, area, &mut self.list_state);
    }

    fn render_applications<B: Backend>(&mut self, f: &mut Frame, area: Rect) {
        let stats = self.tracker.get_statistics();
        let mut app_stats: HashMap<String, u64> = HashMap::new();
        ...
        let rows: Vec<Row> = sorted_apps
            ...
            .map(|(app_name, duration)| {
                ...
                Row::new(vec![
                    ...
                    Cell::from(format!("{}%", (*duration * 100) / self.tracker.get_total_time().max(1))),
                ])
            })
            .collect();
    ...
    fn render_windows<B: Backend>(&mut self, f: &mut Frame, area: Rect) {
        let stats = self.tracker.get_statistics();
        let mut sorted_windows: Vec<_> = stats.iter().collect();

        match (self.sort_by, self.sort_order) {
            (SortBy::Duration, SortOrder::Descending) => sorted_windows.sort_by(|a, b| b.1.cmp(a.1)),
            (SortBy::Duration, SortOrder::Ascending) => sorted_windows.sort_by(|a, b| a.1.cmp(b.1)),
            (SortBy::WindowTitle, SortOrder::Descending) => sorted_windows.sort_by(|a, b| b.0.cmp(a.0)),
            (SortBy::WindowTitle, SortOrder::Ascending) => sorted_windows.sort_by(|a, b| a.0.cmp(b.0)),
        ...
        let rows: Vec<Row> = sorted_windows
            ...
            .map(|(window_name, duration)| {
                ...
                let app_name = parts.get(0).unwrap_or(&"Unknown");
    ...
    fn render_recent<B: Backend>(&mut self, f: &mut Frame, area: Rect) {
        ...
        let rows: Vec<Row> = recent_activities
            ...
            .map(|activity| {
                ...
                Row::new(vec![
                    Cell::from(activity.start_time.with_timezone(&Local).format("%m-%d %H:%M").to_string()),
    ...
    fn render_help<B: Backend>(&self, f: &mut Frame, area: Rect) {
        let help_text = vec![
            Line::from(""),
            Line::from(vec![
            ...
            Line::from(vec![
                Span::styled("  1-4", Style::default().fg(Color::Green).add_modifier(Modifier::BOLD)),
            ...
            Line::from(vec![
                ...
                Span::styled("          - 切换排序方式", Style::default().fg(Color::White)),
            ]),
            Line::from(vec![
                Span::styled("  r", Style::default().fg(Color::Green).add_modifier(Modifier::BOLD)),
                Span::styled("          - 反转排序顺序", Style::default().fg(Color::White)),
            ]),
            Line::from(vec![
            ...
            Line::from(""),
            Line::from(vec![
                Span::styled("标签页:", Style::default().fg(Color::Yellow).add_modifier(Modifier::BOLD)),
            ]),
            Line::from(""),
            Line::from(vec![
                Span::styled("  概览", Style::default().fg(Color::Cyan).add_modifier(Modifier::BOLD)),
                Span::styled("        - 总体统计和当前活动", Style::default().fg(Color::White)),
            ]),
            Line::from(vec![
                Span::styled("  应用程序", Style::default().fg(Color::Cyan).add_modifier(Modifier::BOLD)),
                Span::styled("    - 按应用程序分组的统计", Style::default().fg(Color::White)),
            ]),
            Line::from(vec![
                Span::styled("  窗口", Style::default().fg(Color::Cyan).add_modifier(Modifier::BOLD)),
                Span::styled("        - 详细的窗口使用统计", Style::default().fg(Color::White)),
            ]),
            Line::from(vec![
                Span::styled("  最近活动", Style::default().fg(Color::Cyan).add_modifier(Modifier::BOLD)),
    ...
    fn render_status_bar<B: Backend>(&self, f: &mut Frame, area: Rect) {
        let status_text = vec![
            Line::from(vec![
            ...
            _ => {
                let i = match self.table_state.selected() {
                    Some(i) => {
                        ...
                        if i >= max_items.saturating_sub(1) { 0 } else { i + 1 }
                    }
                    None => 0,
                };
