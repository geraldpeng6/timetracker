...
impl AIConfigManager {
    ...
    pub fn show_config(&self) {
        ...
        if let Some(model_config) = self.config.get_current_model_config() {
            ...
            println!("支持流式输出: {}", if model_config.supports_streaming { "是" } else { "否" });
            println!("支持函数调用: {}", if model_config.supports_function_calling { "是" } else { "否" });
        }
    ...
    pub fn list_models(&self) {
        ...
        for provider in self.config.get_available_providers() {
            ...
            for model in models {
                ...
                println!("  - {}{}{}", model.display_name, current_marker, configured_marker);
    ...
    /// 配置指定厂商
    pub async fn configure_provider(&mut self, provider: &str, model: Option<&String>, api_key: Option<&String>, endpoint: Option<&String>) -> Result<()> {
        // 解析厂商
        ...
        if let Some(key) = api_key {
            if !key.is_empty() {
                self.config.set_api_key(ai_provider.clone(), key.to_string());
        ...
        if let Some(ep) = endpoint {
            if !ep.is_empty() {
                self.config.set_custom_endpoint(ai_provider.clone(), ep.to_string());
        ...
        if let Some(model_name) = model {
            if !model_name.is_empty() {
                ...
                if self.config.models.contains_key(model_name) {
                    ...
                    println!("警告: 模型 {} 不存在，请使用 'timetracker ai list' 查看可用模型", model_name);
                }
            }
        } else {
    ...
    pub fn configure_api_keys(&mut self) -> Result<()> {
        ...
        for provider in self.config.get_available_providers() {
            ...
            print!("\n{} (当前状态: {})\n请输入API密钥: ", provider, current_status);
            io::stdout().flush()?;
            ...
            if !api_key.is_empty() {
                self.config.set_api_key(provider.clone(), api_key.to_string());
    ...
    pub fn select_model(&mut self) -> Result<()> {
        ...
        let mut input = String::new();
        io::stdin().read_line(&mut input)?;
        let provider_index: usize = input.trim().parse()
        ...
        // 检查是否已配置
        if !self.config.is_provider_configured(&selected_provider) && selected_provider != AIProvider::Local {
            println!("警告: 该厂商尚未配置API密钥，请先运行 'timetracker ai config' 进行配置");
        }
        
        // 显示该厂商的可用模型
        let model_entries = self.config.get_model_entries_by_provider(&selected_provider);
        ...
        let mut input = String::new();
        io::stdin().read_line(&mut input)?;
        let model_index: usize = input.trim().parse()
            .map_err(|_| anyhow::anyhow!("无效的输入"))?;
        ...
        println!("\n✓ 已切换到: {} - {}", selected_provider, selected_model_display_name);
        Ok(())
    }
    ...
    pub fn add_custom_model(&mut self) -> Result<()> {
        ...
        let final_api_url = if api_url.is_empty() {
            self.config.get_api_endpoint(&provider)
    ...
    pub fn set_custom_endpoint(&mut self) -> Result<()> {
        ...
        self.config.set_custom_endpoint(provider.clone(), endpoint.clone());
        self.save_config()?;
    ...
    pub async fn test_current_config(&self) -> Result<()> {
        ...
        println!("正在测试: {} - {}", self.config.current_provider, model_config.display_name);
        
        // 检查API密钥
        if !self.config.is_provider_configured(&self.config.current_provider) && 
